...
DropItem.prototype.remove = function(drop) {
  ...
  // debug at pro
  var MANIFEST_URL = 'app://a56d05ee-6347-f047-9f50-1ded42692b80/manifest.webapp';
  // online version
  // var MANIFEST_URL = 'https://john-hu.github.io/fxos-text-battery/manifest.webapp';

  function LiveRainyWallpaper() {
    this.inited = false;
  }

  LiveRainyWallpaper.prototype.init = function() {
    if (this.inited) {
    ...
    scr.insertBefore(container, scr.firstElementChild);
    this.container = container;
    this.inited = true;
    this.updateWallpaper();
  };

  LiveRainyWallpaper.prototype.uninit = function() {
    if (!this.container) {
      return;
    }
  ...
  LiveRainyWallpaper.prototype.updateWallpaper = function() {
    if (!window.wrappedJSObject.Service) {
      console.error('Service is not ready, wait for wallpaperchange');
      return;
    }
    var url = window.wrappedJSObject.Service.query('getWallpaper');
    if (!url) {
      console.error('no url found, wait for wallpaperchange');
      return;
    }
    ...
    img.onload = () => {
      ...
      if (window.wrappedJSObject.appWindowManager.getActiveApp().isHomescreen) {
        this.engine.rain([[3, 3, 1]], 300);
      }
    };
    img.onerror = () => {
  ...
  LiveRainyWallpaper.prototype.handleEvent = function(e) {
    switch(e.type) {
      case 'homescreenopened':
        if (this.engine) {
          console.log('got home, start to rain');
          this.engine.rain([[3, 3, 1]], 300);
        }
        break;
      case 'homescreenclosed':
        if (this.engine) {
          console.log('home closed, stop to rain');
  ...
  function LiveRainyWallpaperMgmt() {
    if (document.documentElement.dataset.liveRainyWallpaper) {
      console.log('live rainy wallpaper is already injected');
      // return;
    }

    this.rainy = new LiveRainyWallpaper();
    this.rainy.init();

    navigator.mozApps.mgmt.addEventListener('enabledstatechange', this);
    navigator.mozApps.mgmt.addEventListener('uninstall', this);

    document.documentElement.dataset.liveRainyWallpaper = true;
  }

  LiveRainyWallpaperMgmt.prototype.handleEvent = function(e) {
    ...
    switch(e.type) {
      ...
      case 'uninstall':
        ...
        document.documentElement.removeAttribute('data-live-rainy-wallpaper');
        break;
    }
  };

  if (document.readyState !== 'loading') {
    new LiveRainyWallpaperMgmt();
  } else {
